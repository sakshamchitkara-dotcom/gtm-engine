...
_REACHED["lost"] = ["contacted"]  # ponytail: lost-after-meeting counts as contacted only; add stage history if needed
...
def render(rows):
    ...
    lines += [f"{st:<12}{n:>6}{c:>8}" for st, n, c in funnel(rows)]
    return "\n".join(lines)
